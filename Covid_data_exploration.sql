...
select location, date, total_cases, population, round((total_cases / population) * 100, 1) as infection_rate
from covid_deaths
where continent <> 'null'
order by 2;
...
select date, sum(new_cases), sum(new_deaths), round((sum(new_deaths) / sum(new_cases)) * 100, 1) as death_percentage
from covid_deaths
where continent <> 'null'
group by 1
order by 1, 2;
...
select sum(new_cases), sum(new_deaths), round((sum(new_deaths) / sum(new_cases)) * 100, 1) as death_percentage
from covid_deaths
where continent <> 'null';
